...
import { addVideo, ADD_VIDEO } from '../../actions'
...
class addVideoModal extends Component {
  ...
  onSubmitForm({name, enName, timeDelay, dama, toloeKhorshid, azanSobh, azanZohr, azanMaghreb, nimeShab}) {
    ...
    this.props.addVideo({name, enName, timeDelay, dama, toloeKhorshid, azanSobh, azanZohr, azanMaghreb, nimeShab, weatherType})
      .then((resp) => {
...
addVideoModal = reduxForm({
	form: 'addVideoModal',
  validate
})(addVideoModal);

export default connect(null, { addVideo })(addVideoModal);
